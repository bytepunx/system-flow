...
func TestIssueCommands(t *testing.T) {
	...
	if code != 0 || !strings.HasPrefix(out, "I-001 Go not on PATH") {
	...
	out, _, code = runIn(t, root, "issue", "bump", "I-001", "--cost", "5m", "--note", "again")
	if code != 0 || !strings.Contains(out, "count 2, avg cost 4m") {
		t.Fatalf("bump: %s", out)
	}
	sum, _ := os.ReadFile(filepath.Join(root, "design", "issues", "summary.md"))
	if !strings.Contains(string(sum), "| [I-001](I-001-go-not-on-path.md) | blocker | Go not on PATH | 2 | 4m | 8m |") {
	...
	out, _, code = runIn(t, root, "issue", "close", "I-001", "--reason", "fixed")
	if code != 0 || !strings.Contains(out, "I-001 closed") {
