...
func TestWorkItemLifecycle(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(out), &created); err != nil || created["id"] != "T-0001" {
		t.Fatalf("task new --json: %v %s", err, out)
	}

	// story needs real criteria before ready
	storyPath := filepath.Join(root, "wip", "kanban", "stories", "S-0001-slice.md")
	b, _ := os.ReadFile(storyPath)
	_ = os.WriteFile(storyPath, bytes.Replace(b, []byte("- [ ]\n"), []byte("- [ ] it works\n"), 1), 0o644)

	for _, step := range [][]string{
		{"move", "S-0001", "ready"}, {"move", "S-0001", "in-progress"},
		{"stream", "open", "S-0001"}, {"stream", "log", "S-0001", "started work"},
		{"move", "T-0001", "ready"}, {"move", "T-0001", "in-progress"}, {"block", "T-0001", "--reason", "waiting"},
		{"unblock", "T-0001"}, {"move", "T-0001", "done"}, {"move", "S-0001", "review"},
	} {
		if _, errOut, code := runIn(t, nested, step...); code != 0 {
			t.Fatalf("%v: %s", step, errOut)
		}
	}
	_, errOut, code = runIn(t, nested, "move", "S-0001", "done")
