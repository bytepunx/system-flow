...
## Acceptance criteria
- [ ] `flai upgrade` in a conforming repo fetches the template (config or `--template`/`--ref`), compares its `version` with `system-flow.yaml` `template.version`, and is a no-op with a clear message when they match unless `--force`
- [ ] Files the template adds are written; files whose rendered content matches what was originally applied are replaced with the new rendering; files the project has modified are reported as conflicts and left alone
- [ ] Conflicts offer keep, replace, or show diff in a terminal; non-interactive runs keep the project's version and exit non-zero with the list unless `--keep-all` or `--replace-all`
- [ ] `CLAUDE.md` is merged by marker: only the section above `<!-- system-flow:end-of-baseline -->` is replaced and everything below is preserved byte for byte
- [ ] Layout renames recorded in `system-flow.yaml` are honoured, so a project with `design` renamed to `architecture` upgrades into the right folders
- [ ] `system-flow.yaml` `template.version` and `template.applied` are updated only when the upgrade completes without unresolved conflicts
- [ ] `--dry-run` prints every planned add, update, conflict, and skip without writing
- [ ] Refuses to run on a dirty git working tree unless `--force`, so the upgrade is reviewable as one diff
- [ ] `flai check` passes on the upgraded repo (once S-008 exists)
