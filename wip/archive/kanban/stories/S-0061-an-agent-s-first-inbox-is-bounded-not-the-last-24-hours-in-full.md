...
updated: 2026-09-19T08:58:24Z
transitions:
  - to: ready
    at: 2026-09-19T06:37:36Z
    by: alex
  - to: in-progress
    at: 2026-09-19T08:14:39Z
    by: system-flow
  - to: review
    at: 2026-09-19T08:19:24Z
    by: system-flow
  - to: done
    at: 2026-09-19T08:58:24Z
    by: alex
tags: [cli]
...
## Acceptance criteria
- [x] `changes` in `inbox` and `events` in `wait_for_events` are capped at a stated number, newest kept, with `changes_omitted` (or the like) saying how many older ones were left out; the cap is a constant with a reason, not a setting
- [x] A first look, with no cursor, reports story and epic changes only, not task transitions, and no more than the cap; ready work and threads are unaffected, since they are state
- [x] After any look the cursor advances past everything, reported or omitted, so nothing omitted comes back later
- [x] Tests: a repository with more changes than the cap, with and without a cursor; the server instructions and `docs/users/flai.md` say what a first look returns
