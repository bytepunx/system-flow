...
- T-0432 Users guide and flai-cli design say how a project imported on the command line reaches the dashboard
...
Delivered on `story/S-0117` (T-0429 to T-0432). Criterion 3: such a repository is served, not offered; why is in the narrative's `## Decisions` and [ADR-0044](../../../design/adrs/0044-a-system-flow-repository-under-a-folder-named-for-import-is-served-whether-or.md). S-0118, in progress at the same time, also edits `flai/internal/serve/serve.go` (`Status` and the status writing in `Run`); whichever is accepted second meets a small conflict there, resolved by keeping both sets of fields and lines. While both are in progress `flai check --strict` warns `wip.overlap`, which fails `make smoke`'s repository check; the template part of the smoke passed.
