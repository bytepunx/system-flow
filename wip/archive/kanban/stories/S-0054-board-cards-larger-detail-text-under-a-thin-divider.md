...
updated: 2026-09-19T06:02:38Z
transitions:
  - to: ready
    at: 2026-09-19T05:35:07Z
    by: alex
  - to: in-progress
    at: 2026-09-19T05:44:05Z
    by: system-flow
  - to: review
    at: 2026-09-19T05:55:43Z
    by: system-flow
  - to: done
    at: 2026-09-19T06:02:38Z
    by: alex
tags: [dashboard]
...
## Acceptance criteria
- [x] The bottom row of `BoardCard.svelte` uses a larger font size than today's 10 pixels; the size is chosen with the operator from a rendered comparison and is the same for every item in the row, including the parent ID
- [x] A thin horizontal line in the theme's line colour separates the title from the bottom row, with even spacing above and below, in the light and the dark theme
- [x] The card still reads well in every column layout: the bottom row stays on one line where it did before or wraps cleanly, the parent stays bottom right, long titles wrap above the line, no card overflows, at desktop, tablet, and phone widths
- [x] The ID and age row at the top is unchanged unless the operator asks for it when choosing the size
- [x] Component test updated for the divider and the row; the board looked at in a browser at three widths in both themes; `docs/users/flaiover.md` updated only if what a card shows changes
