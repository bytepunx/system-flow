...
// (the session cookie is HttpOnly and travels on its own), names the current
// project when there is more than one to choose from (S-0080), and returns
// to the login page when the session is missing or was rotated away.
...
export async function api(path: string, init: RequestInit = {}): Promise<Response> {
	...
	const r = await fetch(projectState.tag(path), { ...init, headers });
