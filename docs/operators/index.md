...
- **Restart** ([ADR-0043](../../design/adrs/0043-flai-serve-starts-a-ready-story-s-agent-whenever-the-in-progress-limit-has-room.md)). `flai serve agent restart <story>`, or **Restart agent** on the story's page, starts a new agent in a new session for a story in ready or in progress whose agent dropped or failed. The new agent is told how the last one ended and goes on from the story's narrative. It refuses, saying why, while the `agent` action is off, when the story is in another state, when `flai serve` started no agent for it, while its agent runs or waits for an answer (an answer starts it again by itself), when nothing can start it, and for a story in ready while the in-progress limit is full. The button runs the command through the `agent.restart` write, gated by the `agent` action and journalled.
