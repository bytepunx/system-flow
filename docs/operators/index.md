...
- **When one starts.** One agent per ready story, in your pull order, while the in-progress limit leaves room. An agent started for a story that is still in ready counts as a story in progress. A story is started once each time it enters ready, from the board, the CLI, or an agent: one whose agent failed waits until it is moved to ready again, or until its agent (harness, model, or options) is changed while it is in ready. While someone else is attending the project, a story waits for them to pull it, for `--attended-minutes` at most.
- **Attending** is judged from files, because flai asks nobody. An agent connected over MCP rewrites its read marker under `.flai-cache/mcp/` at every look, at least every five minutes while it waits. An agent at work writes its story's narrative under `wip/agents/`. If the newest of those is younger than `--attended-minutes` (6 by default), someone is attending and will see the story in their inbox. The marker and narrative of an agent flai started do not count, and a log entry you add to a narrative from the dashboard does, for those minutes. Attending holds a story back for those same minutes and no longer ([ADR-0042](../../design/adrs/0042-someone-attending-holds-a-ready-story-back-for-the-attended-window-then-flai.md)): from the moment attendance is the only thing keeping a story from its agent (the limit has room, and nothing else holds it), the attending agent has the window to pull it, and the story's reason says until when. After that, `flai serve` starts it, whoever is attending: an agent at work on its own story, or you in an interactive session, would otherwise hold every ready story back for as long as you work. The hold is remembered in memory only, so a restart of `flai serve` gives the attending agent the window again. A full in-progress limit is the reason a story waits whoever is attending, and the hold does not begin while the limit is full.
...
- **Why a story waits.** A ready story that has no agent running and is not started is always named with the reason in `waiting` (`flai hostapi agent.status`, and the dashboard): its agent already ran since it entered ready, and how that ended; it names no harness and no command is set; the `agent` action is off; the in-progress limit is full; or someone is attending, with the sign and the time the story has until. `flai serve`'s log has an `agent not started` line with the story and the reason each time a story's reason changes in kind, not each time the age or the time in it moves.
