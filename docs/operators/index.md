...
- **Start** (S-0115). `flai serve agent start <story>`, or **Start agent** on the story's page, starts the agent of a story in ready now, the way `flai serve` does when a story enters ready, whatever its own rules say about when: whether the story was ready before `flai serve` started, whether it has had an agent since it entered ready, and whoever is attending. `flai serve` tracks the run from then on: the dot, the outcome when it ends, the start again on an answer. It refuses, saying why, while the `agent` action is off, when the story is not in ready, while its agent runs or waits for an answer, when nothing can start it (no harness and no command), and while the in-progress limit is full. The story's page offers the button for a ready story that has had no agent, and says why `flai serve` has not started one. The button runs the command through the `agent.start` write, gated by the `agent` action and journalled.
