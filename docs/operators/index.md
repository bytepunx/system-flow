...
- **When one starts.** One agent per ready story, in your pull order, while the in-progress limit leaves room. An agent started for a story that is still in ready counts as a story in progress. A story is started once each time it enters ready, from the board, the CLI, or an agent: one whose agent failed waits until it is moved to ready again. While someone else is attending the project, a story waits for them to pull it, for `--attended-minutes` at most.
