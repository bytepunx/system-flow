...
Before this release, `flai dashboard` started a container per project, each with its own login token and its own agent credential under that project's `.flai-cache`. Since S-0080 ([ADR-0032](../../design/adrs/0032-one-login-token-and-one-agent-credential-per-user-serve-every-project.md)) one container, `flaiover`, serves every project registered with the same `flai serve`, with one shared login token and one shared agent credential kept beside `flai serve`'s own state.
...
Every request except `/_health` and `/_ready` needs the token (ADR-0018), one per user since S-0080, shared by every project the dashboard serves (ADR-0032).
