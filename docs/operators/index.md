...
- **Which projects it serves.** `flai serve project add [folder]` registers one, `flai serve project remove <key|folder>` takes one out without touching its files or stopping the dashboard, and `flai serve project list` shows each with its dashboard address and whether it is connected, the last error, or why it cannot be served (its folder is gone, it has no `system-flow.yaml`, or no key). A project below a folder named for import is served from there whether registered or not; `flai serve import remove` is what stops that. A key another project already has is refused. A project that cannot be served is logged once in `serve.log`, not retried every second, and `flai serve status` names it too. `flai dashboard` in a project registers it the same way (S-0121). The dashboard's Settings page does the same without a shell, once `flai serve enable settings` is on for the project concerned: it lists what `flai serve project list` says, serves a project below an import folder that is not served, and removes a registered one (S-0122).
