...
- **When one starts.** One agent per ready story, in your pull order, while the in-progress limit leaves room. An agent started for a story that is still in ready counts as a story in progress. A story is started once each time it enters ready, from the board, the CLI, or an agent: one whose agent failed waits until it is moved to ready again, until its agent (harness, model, or options) is changed while it is in ready, or until you restart it or start it.
- **Nobody attending holds a story back** ([ADR-0043](../../design/adrs/0043-flai-serve-starts-a-ready-story-s-agent-whenever-the-in-progress-limit-has-room.md)). A story that becomes ready while the in-progress limit has room gets its agent at once, even when you or another agent are connected. An agent holding `wait_for_work` may go for the same story. The second move to in-progress is refused, and the agent that lost the race pulls the next story. To have no agents started, turn the `agent` action off. `--attended-minutes` is retired: it is still accepted, and does nothing.
- **Restart** ([ADR-0043](../../design/adrs/0043-flai-serve-starts-a-ready-story-s-agent-whenever-the-in-progress-limit-has-room.md)). `flai serve agent restart <story>`, or **Retry** on the story's page, starts a new agent in a new session for a story in ready or in progress whose agent dropped or failed. The new agent is told how the last one ended and goes on from the story's narrative. It refuses, saying why, while the `agent` action is off, when the story is in another state, when `flai serve` started no agent for it, while its agent runs or waits for an answer (an answer starts it again by itself), while one is already queued, and when nothing can start it. For a story in ready while the in-progress limit is full, it queues the agent instead ([ADR-0044](../../design/adrs/0044-a-retry-of-a-failed-agent-on-a-ready-story-with-the-in-progress-limit-full-is.md)). The story's dot turns yellow, and `flai serve` starts the agent at its first look with room. The button runs the command through the `agent.restart` write, gated by the `agent` action and journalled.
- **Start** (S-0115). `flai serve agent start <story>`, or **Start agent** on the story's page, starts the agent of a story in ready now, the way `flai serve` does when a story enters ready, whatever its own rules say about when: whether the story was ready before `flai serve` started, whether it has had an agent since it entered ready, and whoever is attending. `flai serve` tracks the run from then on: the dot, the outcome when it ends, the start again on an answer. It refuses, saying why, while the `agent` action is off, when the story is not in ready, while its agent runs or waits for an answer, and when nothing can start it (no harness and no command). A full in-progress limit does not stop it: the agent starts past the limit, with a warning, as a move past it does. The story's page offers the button for a ready story that has had no agent, and says why `flai serve` has not started one. The button runs the command through the `agent.start` write, gated by the `agent` action and journalled.
- **Across a restart.** A restart of `flai serve` (by `flai host` on an upgrade, a host restart, or a crash) forgets nothing that matters: each story's newest run is kept in `serve/agents.json`. The first look after a restart starts each ready story that has had no agent since it entered ready, including one that entered ready while no `flai serve` ran, and never one that has (S-0112, [ADR-0041](../../design/adrs/0041-a-story-in-ready-gets-its-agent-whether-it-entered-ready-before-or-after-flai.md)). An agent that outlived a restart is still watched, and is settled at the next look after it ends. Only projects `flai serve` serves start agents, that is, ones whose dashboard was started with `flai dashboard`.
- **Why a story waits.** A ready story that has no agent running and is not started is always named with the reason in `waiting` (`flai hostapi agent.status`, and the dashboard): its agent already ran since it entered ready, and how that ended; it names no harness and no command is set; the `agent` action is off; or the in-progress limit is full. `flai serve`'s log has an `agent not started` line with the story and the reason each time a story's reason changes in kind, not each time a detail in it moves.
...
- **Where to look.** Each story's card on the board has a dot while its agent runs: green at work, yellow waiting for you (it asked on the story, or the story is blocked), red when it failed. The story's page says which harness and model, since when, the question it waits on, or why it failed and where its log is. `flai serve journal` has every start and failure, and each agent's output is in a log under `serve/agents/` beside `flai serve`'s state. Stopping an agent is yours to do on the host (its PID is in the journal). The dashboard cannot stop or configure one; the starts it can ask for are **Start agent** and **Retry**, under the `agent` action. A yellow dot can also mean a retry is queued for room, or that the story is held in ready because an open story touches the same paths ([ADR-0046](../../design/adrs/0046-a-ready-story-whose-claim-overlaps-an-open-story-s-is-held-yellow-and-with-its.md)); the story's page says which, and a held card says `held` and the story it waits for.
...
### The host host action (S-0106)

`flai host` can restart `flai serve` or the MCP servers, and can install the newest flai and restart everything on it, because the dashboard asked. It is a host action, off until you enable it:

```bash
flai serve enable host      # for the project in the working directory; --all-projects for every project
flai serve journal          # every start, stop, restart, and upgrade asked for, refused ones included
...
- **This project**, with `settings` on here: turn push, agent, checks, dashboard, and host on and off; set the default agent new stories get (written to `system-flow.yaml` and committed); rotate the project's MCP token.
- **Every project**, with `settings` on for all of them: the agent's name and command; each harness's program and the arguments that say what its agent may do; the checks and their time limit; the import folders; the dashboard token. These are kept in your flai configuration for every project, so one project's consent is not enough for them.
...
## flai host: the process that runs the others (S-0106)

`flai host` is one process per machine. It runs `flai serve`, and for each project `flai serve` serves, that project's MCP server over HTTP. They are its children ([ADR-0040](../../design/adrs/0040-one-flai-host-per-machine-runs-flai-serve-and-each-project-s-mcp-server-as-its.md)). `flai dashboard` starts it when it is not running, so you rarely start it yourself.

```bash
flai host status            # the host, and each process it runs: state, PID, version, restarts
flai host start             # in the background; flai host alone runs it in the foreground
flai host restart serve     # serve, mcp, or all; also start and stop <process>
flai host check             # is a newer flai published?
flai host upgrade           # install it, then restart the host and everything it runs on it
flai host stop              # stops the host and every process it runs
```

- **It keeps them running.** A child that ends when nobody asked is started again after a second. The wait doubles while it keeps failing, up to thirty seconds. `flai host status` gives the last exit or error. `flai host stop serve` or `stop mcp` keeps that process stopped until `flai host start serve` (or `mcp`).
- **Nothing outlives it.** Stopping the host stops every child: a SIGTERM, then a kill after ten seconds. A host that is killed outright stops nothing itself, but each child watches it and ends within a few seconds.
- **One per machine.** The host listens on `127.0.0.1:4241`, which is how a second one knows to refuse. The second one names the first one's PID and config file. If another program holds that port, set `FLAI_HOST_ADDR` to another loopback address for every flai you run. The host runs `flai serve` for one flai configuration. `flai dashboard` with another configuration, `FLAI_CONFIG` or `--config`, is refused while it runs.
...
- **Which projects it serves.** `flai serve project add [folder]` registers one, `flai serve project remove <key|folder>` takes one out without touching its files or stopping the dashboard, and `flai serve project list` shows each with its dashboard address and whether it is connected, the last error, or why it cannot be served (its folder is gone, it has no `system-flow.yaml`, or no key). A project below a folder named for import, or below the folder `flai serve` was started in, is served from there whether registered or not; `remove` puts its folder on the list of removed projects (`removed.json`), which `flai serve` does not serve from below a folder, and `add` takes it off (S-0123). `flai serve import remove` stops every project below an import folder at once. A key another project already has is refused. A project that cannot be served is logged once in `serve.log`, not retried every second, and `flai serve status` names it too. `flai dashboard` in a project registers it the same way (S-0121). The dashboard's Settings page does the same without a shell, once `flai serve enable settings` is on for the project concerned: it lists what `flai serve project list` says, serves a project below an import folder that is not served, and removes a registered one (S-0122) or one served from below a folder, and serves it again (S-0123).
- **Where its files are.** A folder named `serve` beside flai's config file: `projects.json` (what it serves), `removed.json` (the projects removed from below a folder, S-0123), `state.json` (rewritten every second while it runs), `serve.log`, and, since S-0080, the login token and the agent credential themselves — nothing in the folder is secret except those two files (mode 0600).
- **Turning it off.** `flai dashboard --no-serve` starts the container without registering the project. The dashboard then has no flai to ask for that project: its board, items, and threads answer 503, every page says why, and `/_ready` reports not ready (its `host_flai` check), which matters if you probe it. Use it when you run `flai serve` yourself in a terminal. `flai serve` run that way, outside `flai host`, keeps no MCP server.
...
`flai host` keeps it running for every project `flai serve` serves ([ADR-0040](../../design/adrs/0040-one-flai-host-per-machine-runs-flai-serve-and-each-project-s-mcp-server-as-its.md), superseding [ADR-0034](../../design/adrs/0034-flai-serve-keeps-each-served-project-s-http-mcp-server-running.md)). It starts one when `flai serve` starts serving a project and starts it again if it stops. It stops it when the project is no longer served, when the host stops, or within seconds of the host being killed. Restarting `flai serve` leaves it running. One you started yourself with `flai mcp start` is used as it is and never stopped by the host. `flai serve status` and `flai host status` show where each project's server listens. The first project gets `127.0.0.1:4243`, the next the next free port, and each keeps its port from then on. For a project `flai serve` does not serve, start it by hand:
...
| | |
|-|-|
| Address | `http://127.0.0.1:4243/mcp` unless `--addr` says otherwise. The address is remembered per project (`.flai-cache/mcp-http.addr`), so an agent's configuration survives a restart; a second project on the same machine needs another port, which `flai host` picks by itself (the next free one from 4243 that no other project remembers). When two projects remember the same port, the first the host starts keeps it and the other moves to a free one, which it remembers from then on. The host's log names the project and both ports (`mcp address moved`), and an agent given the old address over HTTP must be given the new one (S-0110) |
...
| Stopping | `flai mcp stop` ends held `wait_for_events` calls as waits that ran out, so a waiting agent gets an answer, not a broken connection, and simply starts a session again when the server is back. On a server `flai host` keeps, `flai mcp stop` lasts about a second, until the host starts it again. To stop it for good, stop serving the project (`flai dashboard stop`) or run `flai host stop mcp` |
...
The dashboard is built for one person, or a small team on a network they trust. It is not built to face the internet: it speaks plain HTTP, and one token opens everything it serves.

| What | How it stands |
|------|---------------|
| Who can log in | Anyone with the dashboard token (above): one per user, shared by every project that user's `flai serve` holds ([ADR-0033](../../design/adrs/0033-one-login-token-and-one-agent-credential-per-user-serve-every-project.md)). There are no accounts and no roles; every holder acts as you |
| What the token can do | Through `flai serve`, change every project the dashboard serves: moves, edits, new items and ADRs, acceptances, thread entries, all committed with your git identity. With a host action on, what that action does: push and publish with your credentials (`push`), start an agent on this host for a story moved to ready (`agent`), run the review checks (`checks`), restart or upgrade the container (`dashboard`), start and stop flai's processes (`host`), change settings (`settings`), and with `settings --all-projects`, run any command on this host as you ([the settings host action](#the-settings-host-action-changing-the-hosts-settings-from-the-dashboard-s-0105)). Every host action is off until you enable it |
| Where it listens | Every interface of the host at port `4242` by default, so a team can reach it over a private network or VPN. `--bind 127.0.0.1` or `dashboard.bind: 127.0.0.1` keeps it to this machine |
| On the wire | Plain HTTP; the token or the session cookie travels in clear on every request. Beyond a trusted LAN, terminate TLS in a tunnel or proxy and use the `https` address ([the tunnel expectation](#the-tunnel-expectation)) |
| What the container holds | No file of any project, no git, no credential but the two secret files; it can reach the projects only by asking `flai serve` ([ADR-0031](../../design/adrs/0031-the-dashboard-s-container-holds-nothing-of-the-project-a-port-and-two-secrets.md)) |

What to do:

- Never publish port `4242` to the internet or forward it through a router. A public address with this dashboard behind it is a public way to commit to your repositories, and with host actions on, to act on your machine.
- On a machine only you use, bind it to loopback. For access from elsewhere, keep it on loopback and put a tunnel you trust (cloudflared, Tailscale, an SSH forward, a TLS reverse proxy) in front of it.
- Enable host actions only when you want them from the board, and `settings --all-projects` only on a dashboard that nobody else can reach.
- Share the login link only with people you would let commit as you. Rotate the token (`flai dashboard token --rotate`, or from the Settings page) when someone should no longer have it, or when in doubt; rotation ends every other session.
