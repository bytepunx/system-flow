...
**Held stories.** A story in ready is *held* while a story in progress or in review touches the same paths (see `touches` in [flai.md](flai.md)): flai starts no agent for it and offers it to no agent until that story is accepted, cancelled, or sent back. Its card has a yellow dot and, after `BLOCKED` if it is blocked too, a line such as `held (overlap): S-0128`, the reason and the stories it waits for; hover over the dot for flai's whole reason. The story's page says `HELD` beside its state and gives the whole reason at the top of the side column, with each story it names as a link. **Start agent** still starts it, and **Retry** still starts another for one whose agent failed: your word overrides the hold. The card and the page stop showing the hold as soon as it clears, without a reload, and show it whether or not the operator has turned on the `agent` host action.
