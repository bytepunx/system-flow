...
One dashboard can serve several projects on the same host. While it serves only one, nothing here changes. The moment a second one connects, a project switcher appears at the right of the header: pick a project and the page reloads on it. Your choice is remembered in this browser and shown in the page's address, so a bookmark or a link opens on the same project. Nobody sees your inbox, board, or documents mixed with another project's; picking one shows that one and nothing else. If a project you expect is missing, its flai has not connected here yet: ask whoever runs it to run `flai dashboard`.
...
Whether an acceptance has reached the remote is the operator's choice, made on the host: by default it has not, and the board keeps a notice that says what was accepted and not pushed and the command to run on the host (`flai push --pending`), which a running agent session does by itself. Separately, whether it has been **published** — released, with a version bump and a tag — is also the operator's to do, and is shown in the done column itself: a card says **published** or **waiting to publish**, and a banner at the top of the column lists what publishing now would release, by component, with the story IDs it bundles (several small stories against the same component release together, at the highest bump among them, not one release each). When the operator has enabled pushing from the board (`flai serve enable push`), the banner has a **Publish** button; off, it says what enables it. Publishing applies, tags, and pushes together; a publish that fails partway is safe to run again, it does not redo what already succeeded. The standing "accepted, not pushed" notice keeps its own Push now button for ordinary merged commits, released or not; a push that is refused, because the remote has moved, shows its reason and the command to run by hand. While the clone holds an acceptance that has not been pushed, the board and that story's page show a standing notice: which stories, how many commits ahead of the remote, and the command to run on the host (`flai push --pending`). It stays on every load and clears by itself once the push has happened from this clone; a push made from another clone is not seen until someone fetches here. Every write is made by the bundled `flai`, so the files change exactly as they would from the terminal and appear in `git status` for you to commit.
