...
**Held stories.** A story in ready is *held* while a story in progress or in review touches the same paths (see `touches` in [flai.md](flai.md)): flai starts no agent for it and offers it to no agent until that story is accepted, cancelled, or sent back. It is held too while a story it waits for, named in its **Waits for** field, is not done (`held (after): S-0129`). Its card has a yellow dot and, after `BLOCKED` if it is blocked too, a line such as `held (overlap): S-0128`, the reason and the stories it waits for; hover over the dot for flai's whole reason. The story's page says `HELD` beside its state and gives the whole reason at the top of the side column, with each story it names as a link. **Start agent** still starts it, and **Retry** still starts another for one whose agent failed: your word overrides the hold. The card and the page stop showing the hold as soon as it clears, without a reload, and show it whether or not the operator has turned on the `agent` host action.
...
An agent working on the story is told. Its `inbox` reports the edit with what changed (title, nature, tags, touches, after, parent, goal, criteria, notes, or body), and one that is waiting hears within a second, so it reads the story again before it goes on.
