...
Projects lists every project `flai serve` serves, with its key, name, and folder, and whether it is connected and since when, the last error, or why it cannot be served. A project below an import folder is served as soon as `flai serve` next looks, and joins the switcher without a reload. One it does not serve is listed under "Below the import folders, not served" with why: its `system-flow.yaml` has no key, or its key is served already for another folder. **Serve** asks `flai serve` to register it, as `flai serve project add` does, and shows the answer; for those two reasons that is a refusal saying what to fix. **Remove**, on a registered project, asks first, then unregisters it, as `flai serve project remove` does: none of its files is touched, and the page says how to serve it again. A project served because it is below an import folder, or below the folder `flai serve` was started in, has no Remove: it says which folder serves it, and removing that folder under Import folders stops it. Serve and Remove need `flai serve enable settings` for the project served or removed, and each says so, with the command, where it is off.
