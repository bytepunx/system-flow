...
Acceptance checks what could fail midway before it changes anything: without a git committer identity it refuses and the story stays in review. A push that cannot happen, for example from the dashboard container, which has no credentials unless the operator names a push key (`dashboard.push_key`, [operators](../operators/index.md)), does not undo the acceptance: the result says it was accepted locally and not pushed, and `git push origin HEAD --follow-tags` from a shell finishes the job. A story that is `done` but was never accepted (an older flai, a hand edit) is flagged by `flai check` as `story.unaccepted`, and `flai accept` completes it.
