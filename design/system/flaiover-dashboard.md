...
| `/board` | Kanban board: one column per state with WIP count against the limit, story cards (epics and tasks on request) with age in column, nature, blocked flag, and the parent's ID bottom right with its title as tooltip and accessible name (`BoardCard.svelte`, S-0048), that detail row in one size under a thin divider, wrapping when crowded (S-0054); drag to transition, refused moves show the rule; refreshes on SSE (S-0013) | `/api/board`, `POST /api/items/:id/move` |
