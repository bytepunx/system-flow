...
- **Pushing (S-0078).** `GET /api/unpushed` also answers `push_enabled`, asked of flai each time. `POST /api/unpushed` is flai's `push.run`, a host action: 403 with what enables it while the operator has not (`flai serve enable push`, on the host), 409 with the reason when the remote has moved, else what was pushed and published. `UnpushedNotice` offers **Push now** only when enabled, shows `Pushing:`, then what was pushed or why not with the command to run by hand; off, it says what enables it. `Review` says before an acceptance whether it will push, shows `pushing` as a step, and ends with pushed or not pushed and the command. Nothing in the dashboard can enable an action. Until this story an acceptance from the board pushed unasked (I-0028): `accept.run` ran on the host without `--no-push`.
