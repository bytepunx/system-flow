...
`summary.md` is a table of open issues: ID, class, title, count, average cost per occurrence, total cost (count times average), last reported. Sorted by total cost descending so the most expensive friction is first. It is regenerated by `flai issue` (S-027); until then it is maintained by hand with every occurrence.
