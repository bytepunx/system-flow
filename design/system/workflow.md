...
- **Held.** A ready story is held when its claim overlaps an open story's by the `/`-prefix rule `flai check` uses (`overlap`), or when its own claim or an open story's is empty (`no-touches`).
...
S-0129 built the dashboard's yellow card: a held story's card and page say why it waits and link the story it waits for ([flaiover-dashboard.md](flaiover-dashboard.md)). Still to come under E-0009, in this order: `after:`, the trial merge and drift check at `flai stream sync`, and the notice at acceptance.
