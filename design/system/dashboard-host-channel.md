...
5. S-0076: MCP served by flai on the host, over HTTP as well as stdio; the dashboard's `/mcp` removed. Landed as [ADR-0030](../adrs/0030-mcp-is-served-by-flai-on-the-host-over-stdio-and-http-and-the-dashboard-s-api.md): a process of its own per project (`flai mcp start`), not part of `flai serve`, serving revisions with and without sessions at one address. Since S-0096 `flai serve` keeps that process running for each project it serves ([ADR-0034](../adrs/0034-flai-serve-keeps-each-served-project-s-http-mcp-server-running.md)), as a child it stops with itself.
...
8. S-0079: an agent started when a story becomes ready. Landed as the host action `agent`, performed by `flai serve` itself from what the watcher sees, with attending judged from MCP cursors and narratives. The attending hold was capped by ADR-0042 and dropped by [ADR-0043](../adrs/0043-flai-serve-starts-a-ready-story-s-agent-whenever-the-in-progress-limit-has-room.md) (S-0116), which also added `agent.restart`, a method gated by `agent`; S-0115 added the other, `agent.start`.
9. S-0080: one dashboard for every project. Landed as [ADR-0033](../adrs/0033-one-login-token-and-one-agent-credential-per-user-serve-every-project.md): one login token and one agent credential per user, kept beside `flai serve`'s state rather than a project's `.flai-cache`; the container fixed at one name (`flaiover`) that any project registers with; `AgentRegistry` on the dashboard side routing by the project key each connection's `hello` names, with a `solo()` fallback so a dashboard with at most one project needs no configuration change; a project switcher, remembered per browser, once a second project connects.
10. S-0081: the dashboard managed from its own page. The host action `dashboard` gates `dashboard.restart`, `dashboard.upgrade`, and `dashboard.stop`, a fourth instance of the host-actions pattern ADR-0029 established (push, agent, now dashboard), not a new one; `dashboard.status` and `dashboard.check` are reads, ungated, matching `push.pending`'s precedent of a read that does real work (a pull, to compare images). An upgrade is never applied until the pulled image, started as a second, temporary container of its own, answers healthy: only then is the running container stopped, so a bad image leaves it untouched. Found only against real Docker (T-0325, not by any fake-runner unit test): a request whose own success stops the container it arrived on — restart always, an upgrade that swaps, a stop that was the last project — had its command killed mid-run, because the request's context is a child of the WebSocket connection's, and that connection dies as a direct side effect of the command it is still running. `dashboard.restart`, `dashboard.upgrade`, and `dashboard.stop` now run with a background context of their own (`spec.detachTimeout`, `internal/hostapi`), not the request's; every other write is unaffected. `HostPanel.svelte` has the matching client-side half: it expects these three writes to answer with a network error, not a response, races each against a short timeout, and polls status until the dashboard is reachable again.
11. S-0082, in progress: checks run for a story in review. The host action `checks` gates `checks.run` and `checks.cancel`, a fifth instance of the host-actions pattern (`spec.detachTimeout` again, matching S-0081's lesson: an operator closing the review page mid-run must not kill it, so `checks.run` gets a generous 2-hour ceiling — the real bound is the operator's own `flai serve checks timeout`, enforced inside `flai checks run` itself). `checks.status` and `checks.tail` are reads, ungated; `checks.tail`'s progress events are the run's own log lines, streamed with the same `spec.progress` machinery a `$/progress`-carrying write already uses, not a new transport.
12. S-0098: a repository imported from the board. Landed as [ADR-0035](../adrs/0035-repositories-under-folders-the-operator-names-can-be-imported-from-the-board.md): `flai serve` looks in the folders the operator names with `flai serve import add` for git repositories without a manifest and opens a connection for each, whose hello says `kind: candidate`, answering only `import.preview` and `import.run` (`flai import --commit`, journalled as `import`, exit 5 as `NotCommitted`, -32013). The dashboard lists a candidate as not imported while it is connected, never counts it as missing methods or as the one project, and shows `ImportPrompt` in place of any page for it. Once imported, `flai serve` registers the repository and serves it as a project.
13. S-0118: a project removed from `flai serve` leaves the switcher. The dashboard keeps a hub for every project that ever connected and lists it as not connected once its flai goes, which is right for a flai that is restarting and wrong for a project no longer served. Before `flai serve` drops the connection of a project that left its registry (`flai serve project remove`, `flai dashboard stop`) or is now served under another key, it sends the notification `removed` with the project's key. The hub marks itself removed, `list()` and `solo()` leave it out once it is not connected, as they do a gone candidate, and a new connection for the key clears the mark. An older dashboard ignores the notification and an older flai never sends it, so neither side needs the other upgraded.
