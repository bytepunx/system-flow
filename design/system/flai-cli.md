...
| `flai accept <id> [--by] [--deliver] [--no-release] [--no-push] [--trailer]` | The operator's acceptance in one step: rebase and fast-forward the story branch into main and remove its worktree, move to done, archive, template bump, commit, tags on that commit, push. Uncommitted changes under `wip/` are expected; anything else needs `--yes`. |
...
| `flai thread new\|reply\|resolve\|list` (E-0006) | Threads under `wip/threads/` anchored to a document, heading, or item. |
