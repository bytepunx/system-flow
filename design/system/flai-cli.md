...
| `flai migrate ids [--dry-run]` | Widen every work item ID to four digits: rename items and narratives in kanban and archive with `git mv` (plain rename outside git) and rewrite references in the layout folders, the root markdown and yaml files, and each project's root markdown files. Idempotent. |
