...
| `flai release <id> [--dry-run] [--deliver] [--apply]` | Compute the semver release for an item per the git convention (delivered component gets the delivery-type bump, touched components a patch) and with `--apply` create the tags and version bumps. A `research` story plans no release ([ADR-0025](../adrs/0025-research-is-accepted-without-a-release.md)): `skipped` gives the reason and `unreleased` lists `{ component, files }` for each component its commits touched; an `experiment` story is an error. |
