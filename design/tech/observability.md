...
|-------------|---------|-----------|-------|
| flai | Go standard library `log/slog`: text handler on a terminal, JSON handler otherwise, to stderr; custom fatal level above error; keys renamed to `ts`, `level`, `msg`. Implemented in `flai/internal/logx` (S-0031). | None by default; an optional root span via the OpenTelemetry Go SDK only when `OTEL_*` is set, decided in the story. | S-0031 |
| flaiover | `pino` 10.3 (JSON lines to stdout; `pino-pretty` 13 as a dev dependency for `LOG_FORMAT=text`); request logging in `src/hooks.server.ts`. | `@opentelemetry/api` 1.9, `sdk-node` and `exporter-trace-otlp-http` 0.222, `resources` 2.11, `semantic-conventions` 1.43 for traces; `@prometheus-io/client` 0.16 (the maintained successor of the deprecated `prom-client`) for `/metrics`; `/_health` and `/_ready` are plain routes. Local stack `grafana/otel-lgtm` via `flaiover/compose.yaml`. | S-0032 |
| template | Ships the conventions only; no library. | Compose stack for local testing gains an OpenTelemetry collector and a viewer when the first service needs one. | S-0032 |
