...
## Project additions
- Operator documentation for flai and flaiover meets the settings-index and runbook rules above since S-0028: `docs/operators/settings.md` indexes every setting (its flag table is generated by `make flai-reference`, and a test fails when a configuration key, manifest key, or environment variable has no row), and `docs/operators/runbooks/` has install, update, backup, restore, migrate, and delete. A change that adds a setting or changes how flai keeps its state updates them in the same story.
