...
## Project additions
- flai logs with the standard library `log/slog` to stderr: text handler on a terminal, JSON otherwise; `--verbose` selects debug; `LOG_LEVEL` and `LOG_FORMAT` are honoured. Component is the Go package. Implemented in S-031; until then flai prints plain messages to stderr.
