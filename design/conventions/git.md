...
- A `research` story is accepted like any other, and its findings (ADRs, design documents, new stories) land on main and are pushed, but it cuts no release whatever files it touched: a finding is a deliverable of the repository, not of a component. When its commits touched a component's files, the release plan says that component lands on main without a release. An `experiment` story stays on a branch and gets its additional testing there; acceptance refuses it. Whenever an acceptance cuts no release, the acceptance commit is still pushed. Releasing research builds with semver pre-release suffixes (`1.3.0-rc.1`) is a pattern to adopt when needed and is not yet defined.
