...
- Check the MCP inbox (`flai mcp`, registered in `.mcp.json`) at session start, at every task transition, and before moving a story to review; hold `wait_for_events` when idle. Answer threads with `thread_reply` and ask the designer with `thread_open` rather than stopping to ask in the conversation when the designer is not watching.
