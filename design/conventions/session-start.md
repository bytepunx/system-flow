...
- If `FLAI_STORY` is set in your environment, flai started this session on the host because that story became ready and nobody was attending the project (`FLAI_STARTED_BY` says `flai-serve`). Prime as always, then call `inbox`: pull that story if it is still ready and the limit allows, otherwise the first ready story, otherwise nothing. Work as in any session. When nothing is left for you to pull, end the session instead of idling: flai starts one agent per project at a time, and the next ready story waits for yours to end.
...
## Project additions
- This repository builds itself. `template/` is the development copy of the template; `system-flow.yaml` points flai at it. Read `design/system/overview.md` once per session for the four parts and the delivery order.
- Use the wrapper `scripts/flai.sh` (builds `bin/flai` from source if missing and sets `FLAI_CONFIG` to `.flai-cache/config.json`) so the CLI you run is the one in this tree.
- Set `FLAI_AGENT` to the model name and `FLAI_SESSION` to the session ID before the first flai call.
