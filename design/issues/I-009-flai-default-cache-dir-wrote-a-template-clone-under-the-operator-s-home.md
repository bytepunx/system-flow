...
S-006: a test resolved a relative template path against the test's working directory, treated it as a remote, and created ~/.flai/cache. Removed. Config now honours FLAI_CACHE_DIR; test helpers and scripts/env.sh set it so neither tests nor scripts can reach the home directory.
